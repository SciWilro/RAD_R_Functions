...
## Calculate the SNP and allele population specific frequencies according popRef Major allele Freq ---------
    ...
    #standardize the allele frequency
      ...
      #if the optimizer is going to work we will have to play around with some general assumptions.
      ...
      if(standardize & optimizer)
        ...
        SNPS <- HeatMapData%>%group_by(SNP,Pop)%>%summarise(Mean=mean(opt_FreqStand,na.rm=T))%>%ungroup()%>%data.frame()
        northSNPS <- HeatMapData%>%filter(Pop %in% northPop)%>%group_by(SNP)%>%
          summarise(Mean=mean(opt_FreqStand,na.rm=T))%>%ungroup()%>%data.frame()
        southSNPS <- HeatMapData%>%filter(Pop %in% southPop)%>%group_by(SNP)%>%
          summarise(MeanS=mean(opt_FreqStand,na.rm=T))%>%ungroup()%>%data.frame()
...
#snp Optimizer
      if(snpOptimizer){
        ...
        nSnp <- filter(HeatMapData2,NorthSouth=="North")%>%group_by(SNP)%>%summarise(mFreq=mean(FreqStand,na.rm=T))%>%
                ungroup()%>%data.frame()%>%arrange(.,mFreq)
        
       HeatMapData$SNP=factor(HeatMapData$SNP,levels=nSnp$SNP)
      }
