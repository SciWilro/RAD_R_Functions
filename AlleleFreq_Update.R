    ...
    #standardize the allele frequency
      ...
      if(standardize & optimizer)
        ...
        northSNPS <- data.frame(HeatMapData%>%filter(Pop %in% northPop)%>%group_by(SNP)
                              %>%summarise(Mean=mean(opt_FreqStand,na.rm=T))%>%ungroup())
        southSNPS <- data.frame(HeatMapData%>%filter(Pop %in% southPop)%>%group_by(SNP)
                              %>%summarise(MeanS=mean(opt_FreqStand,na.rm=T))%>%ungroup())
